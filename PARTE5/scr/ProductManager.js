...
class ProductManager {
    constructor(){
        this.path = './public/json/products.json';
    }
 ...
 class ProductoData extends ProductManager {
  ...
  async getProductsById(id){
    const product= JSON.parse( await fs.readFile(this.path, "utf-8"));
    const idExists = product.products.find((product) => product.id ===id);
    const prodActive = product.products.filter((prod)=> prod.status === true)
    const allGood = prodActive.some((prod)=> prod.id ===id)

    if( idExists && allGood){
      return {success: true, message: idExists}
    } else {return {success: false, message: "No existente"}}}
  ...
  async addProducts(title, description, price, thumbnail, code, stock) {
    ...
    else if(!title || !description || !price || !code || !stock) {return{success: false, message: "Es necesario completar todos los datos para agregar un producto."}}
    else {
  ...
  async updateProduct(id, product){
   ...
   async deleteProducts(id){
    ...
    if (deleteProd){ 
        ...
        await fs.writeFile(this.path, JSON.stringify(data))
        return {success: true, message:"Se ha eliminado exitosamente el producto"}
    } else {
